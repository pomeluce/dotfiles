...
const AppItem = (address: string) => {
  ...
  const btn = PanelButton({
    ...
    tooltipText: Utils.watch(client.title, hyprland, () => hyprland.getClient(address)?.title || ''),
    onPrimaryClick: () => focus(address),
    onMiddleClick: () => app && launchApp(app),
    child: Widget.Icon({
      size: iconSize.bind(),
      icon: monochrome.bind().as(m => icon((app?.icon_name || client.class) + (m ? '-symbolic' : ''), icons.fallback.executable)),
    }),
  });
  ...
  return Widget.Box(
    ...
    Widget.Overlay({
      ...
      overlay: Widget.Box({
        className: 'indicator',
        hpack: 'center',
        vpack: position.bind().as(p => (p === 'bottom' ? 'start' : 'end')),
        setup: w =>
          w.hook(hyprland, () => {
