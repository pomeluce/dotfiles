...
const MicrophoneIndicator = () =>
  Widget.Icon()
    .hook(audio, self => (self.visible = audio.recorders.length > 0 || audio.microphone.stream?.is_muted || audio.microphone.is_muted))
...
export default () =>
  PanelButton({
    ...
    child: Widget.Box([DNDIndicator(), BluetoothIndicator(), NetworkIndicator(), AudioIndicator(), MicrophoneIndicator()]),
  });
