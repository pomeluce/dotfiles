...
export default function init() {
  options.wallpaper.name.connect('changed', () => matugen());
  options.autotheme.connect('changed', () => matugen());
}

const animate = (...setters: Array<() => void>) => {
...
const defualt = (dir.value.endsWith('/') ? dir.value : dir.value + '/') + name.value;

export const matugen = async (type: 'image' | 'color' = 'image', arg = defualt) => {
